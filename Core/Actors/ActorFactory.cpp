...
namespace liman {
	...
	ActorFactory::ActorFactory() {
		...
		m_pCompFactory->RegisterComponent<TransformComponent>(
				ActorComponent::GetIdFromName(TransformComponent::g_Name));
		m_pCompFactory->RegisterComponent<Renderable>(
				ActorComponent::GetIdFromName(Renderable::g_Name));
	}
	...
	Actor* ActorFactory::CreateActor(tinyxml2::XMLElement* pActorNode, std::string &sourceName) {
		...
		auto pCompsNode = pActorNode->FirstChildElement("Components");
		if (!pCompsNode) {
			LOG("Actor Factory", "No components are specified");
		} else {
			for (tinyxml2::XMLElement* pCompNode = pCompsNode->FirstChildElement(); pCompNode;
				 pCompNode = pCompNode->NextSiblingElement()) {
		...
		auto* pRend = pActor->GetComponent<Renderable>(Renderable::g_Name);
