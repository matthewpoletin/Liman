...
namespace liman {
	...
	void Renderable::UpdateMesh() {
		if (!m_pOwner) {
			LOG("Renderable", "Host not found");
			return;
		}
		auto* pTrans = m_pOwner->GetComponent<TransformComponent>(TransformComponent::g_Name);
		if (!pTrans) {
			LOG("ActorFactory", "Transform component not found");
			return;
		}
		...
		Mesh* pMesh = new Mesh(vertices, sizeof(vertices) / sizeof(vertices[0]), indices,
